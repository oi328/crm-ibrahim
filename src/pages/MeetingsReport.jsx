        ...
        <div className="flex items-center justify-between relative">
          ...
          {showFilter && (
            <div className="absolute right-0 top-full mt-2 z-20 glass-panel rounded-xl p-4 w-80 shadow-lg">
              <div className="text-sm font-semibold mb-2">{isRTL ? 'تصفية البيانات' : 'Filter Data'}</div>
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="col-span-2">
                  <label className="text-xs mb-1 block">{isRTL ? 'مندوب المبيعات' : 'Salesman'}</label>
                  <input value={filters.salesman} onChange={(e)=>setFilters(f=>({...f, salesman: e.target.value}))} className="input w-full text-sm" placeholder={isRTL ? 'اكتب الاسم' : 'Type name'} />
                </div>
                <div className="col-span-2">
                  <label className="text-xs mb-1 block">{isRTL ? 'المشروع' : 'Project'}</label>
                  <input value={filters.project} onChange={(e)=>setFilters(f=>({...f, project: e.target.value}))} className="input w-full text-sm" placeholder={isRTL ? 'اكتب المشروع' : 'Type project'} />
                </div>
                <div>
                  <label className="text-xs mb-1 block">{isRTL ? 'من تاريخ' : 'From Date'}</label>
                  <input type="date" value={filters.dateFrom} onChange={(e)=>setFilters(f=>({...f, dateFrom: e.target.value}))} className="input w-full text-sm" />
                </div>
                <div>
                  <label className="text-xs mb-1 block">{isRTL ? 'إلى تاريخ' : 'To Date'}</label>
                  <input type="date" value={filters.dateTo} onChange={(e)=>setFilters(f=>({...f, dateTo: e.target.value}))} className="input w-full text-sm" />
                </div>
              </div>
              <div className="flex items-center justify-end gap-2">
                <button className="btn btn-glass" onClick={()=>setFilters({ salesman:'', project:'', dateFrom:'', dateTo:'' })}>{isRTL ? 'مسح' : 'Clear'}</button>
                <button className="btn bg-primary text-white" onClick={()=>setShowFilter(false)}>{isRTL ? 'تطبيق' : 'Apply'}</button>
              </div>
            </div>
          )}
        </div>
