...
export default function SearchableSelect({
  ...
  return (
    <div ref={ref} className={`relative ${disabled ? 'opacity-60 pointer-events-none' : ''}`}>
      ...
      {open && createPortal(
        <div ref={menuRef} style={menuStyle} className="rounded-md border border-base-300 bg-[var(--content-bg)] shadow-xl">
          ...
          <ul role="listbox" className="max-h-44 overflow-auto py-1">
            ...
            {filtered.map((o) => (
              <li
                ...
                className={`px-3 py-2 text-sm cursor-pointer hover:bg-[var(--table-row-hover)] ${o.value === value ? 'bg-[var(--table-row-hover)]' : ''}`}
