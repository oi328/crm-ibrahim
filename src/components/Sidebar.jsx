...
const isCustomersActive = location.pathname.startsWith('/customers') || location.pathname.startsWith('/sales') || location.pathname.startsWith('/support')
...
useEffect(() => { if (isDataMgmtActiveFlag) { openOnly('dataMgmt') } else { setDataMgmtOpen(false) } }, [isDataMgmtActiveFlag])
  ...
  const isSectionViewOpen = leadMgmtOpen || inventoryOpen || marketingOpen || customersOpen || reportsOpen || settingsOpen || usersOpen
    ...
    >
      ...
      <button
        ...
        onClick={(e) => { 
          e.preventDefault(); 
          e.stopPropagation(); 
          console.log('🔴 Close button clicked - السايدبار سيتم إغلاقه الآن');
          ...
          if (onClose) {
            ...
            console.error('❌ onClose function is not available!');
          }
        }}
        className={`md:hidden absolute top-4 ${isRTL ? 'left-4' : 'right-4'} z-[60] p-3 rounded-full bg-red-500 hover:bg-red-600 active:bg-red-700 focus:outline-none focus:ring-4 focus:ring-red-300 shadow-xl transition-all duration-200 border-2 border-white`}
        style={{ zIndex: 9999 }}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
      ...
      {/* Spacer below brand to push menu down */}
        ...
        {!isMarketingActive && !isCoreReportsActive && (!isSectionViewOpen || leadMgmtOpen) && activeModules.includes('leads') && (
          ...
          {!leadMgmtOpen && (
            ...
            >
              <span className="nova-icon-label">
                ...
                <span className="link-label">{t('Lead Management')}</span>
              </span>
          ...
          >
            ...
            <div className={`mt-1 ${isRTL ? '!pr-0' : '!pl-0'}`}>
              <div className="py-1">
                <span className="text-sm font-semibold tracking-wide text-gray-700 dark:text-gray-200">{t('Stages')}</span>
              </div>

              <div className={`inline-block w-64 rounded-md ${isLight ? 'bg-blue-50 border border-blue-100' : 'bg-gray-800 border border-gray-700'} p-2`}>
                <div className="space-y-1">
                  {/* All Leads card */}
                  <NavLink
                    to={location.pathname.startsWith('/recycle') ? '/recycle' : '/leads'}
                    className={({ isActive }) => `block w-full relative overflow-hidden rounded-md border p-2 shadow-sm hover:shadow-md transition-all ${isLight ? 'bg-white/90' : 'bg-gray-900/60'} border-blue-400 dark:border-blue-500`}
                    style={{ ...( !currentStageParam ? { borderColor: '#3B82F6' } : {}) }}
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        <span className={`${iconContainer} !w-6 !h-6 rounded-lg bg-blue-600 dark:bg-blue-500`}>
                          <span className="text-sm">📊</span>
                        </span>
                        <span className="text-sm font-semibold link-label">{langCode.startsWith('ar') ? 'الكل' : 'All Leads'}</span>
                      </div>
                      <div className="text-xs font-bold bg-white/70 dark:bg-gray-700/70 px-2 py-0.5 rounded-md border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300">
                        100%
                      </div>
                    </div>
                    <div className="mt-1 text-lg font-black text-gray-900 dark:text-white">{currentLeadsDataset.length}</div>
                  </NavLink>
                {safeStages && safeStages.length > 0 ? safeStages.map((stage, idx) => {
                  const stageName = stage?.name || String(stage)
                   const label = (langCode.startsWith('ar') ? (stage?.nameAr || stageName) : stageName)
                   const targetBase = location.pathname.startsWith('/recycle') ? '/recycle' : '/leads'
                   const toUrl = `${targetBase}?stage=${encodeURIComponent(stageName)}`
                   const isActiveStage = currentStageParam === stageName
                   const count = currentLeadsDataset.filter(l => (l.stage || l.status || '').toLowerCase() === stageName.toLowerCase()).length
                   const total = currentLeadsDataset.length
                   const percent = total > 0 ? Math.round((count / total) * 100) : 0
                   const color = stage?.color || '#3B82F6'
                   const isHex = isHexColor(color)
                   const containerStyle = isHex ? { borderColor: color, background: `linear-gradient(to bottom right, ${withAlpha(color, 0.12)}, ${withAlpha(color, 0.24)})` } : undefined
                   const stylePreset = !isHex ? (COLOR_STYLES[color] || COLOR_STYLES.blue) : null
                   const iconClass = isHex ? '' : (stylePreset?.iconBg || '')
                   const borderStyle = isActiveStage ? { borderColor: color } : undefined

                  return (
                    <NavLink
                      key={`stage-${idx}-${stageName}`}
                      to={toUrl}
                      className={({ isActive }) => `block w-full relative overflow-hidden rounded-md border p-2 shadow-sm hover:shadow-md transform hover:scale-[1.01] transition-all ${isLight ? 'bg-white/90' : 'bg-gray-900/60'} ${stylePreset?.container || ''}`}
                      style={{ ...(containerStyle || {}), ...(borderStyle || {}) }}
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className={`${iconContainer} !w-6 !h-6 rounded-lg ${iconClass}`} style={isHex ? { backgroundColor: color } : undefined}>
                            <span className="text-sm">{stage?.icon || '📊'}</span>
                          </span>
                          <span className="text-sm font-semibold link-label">{label}</span>
                        </div>
                        <div className="text-xs font-bold bg-white/70 dark:bg-gray-700/70 px-2 py-0.5 rounded-md border border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300">
                          {percent}%
                         </div>
                      </div>
                      <div className="mt-1 text-lg font-black text-gray-900 dark:text-white">{count}</div>
                    </NavLink>
                  )
                }) : (
                  <div className={`text-xs ${isLight ? 'text-gray-500' : 'text-gray-400'} ${isRTL ? '!pr-10' : '!pl-10'}`}>{t('No stages configured')}</div>
                )}
              </div>
          ...
          {inventoryOpen ? (
            <div className={`sticky top-0 z-10 flex items-center justify-between mb-2 ${isLight ? 'bg-gray-100' : 'bg-gray-900'} px-2 py-1`}>
              ...
              <button
                ...
                className={`ml-auto text-sm font-semibold ${isLight ? 'text-gray-700 hover:text-gray-900' : 'text-gray-200 hover:text-white'} flex items-center gap-2`}
              >
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-4 h-4">
                  <path d="M15 18l-6-6 6-6" />
                </svg>
            ...
            >
              <span className="nova-icon-label">
                ...
                <span className="link-label">{t('Inventory')}</span>
              </span>
        ...
        {!isMarketingActive && !isCoreReportsActive && (!isSectionViewOpen || customersOpen) && (
          <div className="w-full">
            {customersOpen ? (
              <div className={`sticky top-0 z-10 flex items-center justify-between mb-2 glass-header glass-neon px-2 py-1`}>
            ...
            >
              ...
              {/* Support nested submenu under Customers */}
              <div className="w-full">
                <button
                  type="button"
                  onClick={() => setSupportOpen(v => !v)}
                  className={`${baseLink} w-full justify-between ${isRTL ? '!pr-0' : '!pl-0'} ${isSupportActive ? 'active-parent' : ''}`}
                  aria-expanded={supportOpen}
                >
                  <span className="nova-icon-label">
                    <span className={`${iconContainer} ${iconTone}`}>{getIcon('Support')}</span>
                    <span className="text-[15px] link-label">{t('Support')}</span>
                  </span>
                ...
                <div
                  ...
                  style={{ maxHeight: supportOpen ? '600px' : '0', overflow: 'hidden', opacity: supportOpen ? 1 : 0 }}
                >
                  <NavLink
                    to="/support"
                    end
        ...
        {/* Support section moved under Customers submenu */}
      ...
      {!isMarketingActive && !isCoreReportsActive && (!isSectionViewOpen || settingsOpen) && activeModules.includes('settings') && (
        <div className="pt-2 w-full">
          <div className={`border-t ${isLight ? 'border-gray-200' : 'border-gray-800'} mb-3`}></div>
            {settingsOpen ? (
              <div className={`sticky top-0 z-10 flex items-center justify-between mb-2 bg-indigo-50 dark:bg-indigo-900/40 px-3 py-1 rounded-md`}>
              ...
              >
                <span className="nova-icon-label">
                  ...
                  <span className="link-label">{t('Settings')}</span>
                </span>
